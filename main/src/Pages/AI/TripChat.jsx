...
import { MessageSquare, Compass, MapPin, Lightbulb, X, RotateCcw, Send } from "lucide-react";

const TripChat = () => {
    const [isOpen, setIsOpen] = useState(false);
    const [messages, setMessages] = useState([
        { id: 1, text: "Hello! I'm **Trippy 2.0**, your AI travel assistant. Ask me about destinations, travel tips, or anything about the platform. Where shall we go?", isUser: false }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages(prev => [
                ...prev,
                { id: Date.now() + 1, text: "Oops! Something went wrong. Please try again later.", isUser: false }
            ]);
        } finally {
            setIsTyping(false);
        }
    ...
    const quickActions = [
        { label: "Destinations", icon: <MapPin size={12} />, msg: "Suggest some amazing travel destinations" },
        { label: "Travel Tips", icon: <Lightbulb size={12} />, msg: "Give me some travel tips" },
        { label: "Navigate", icon: <Compass size={12} />, msg: "Help me navigate the platform" },
    ];
                ...
                <div className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] h-[520px] max-h-[calc(100vh-8rem)] flex flex-col rounded-3xl overflow-hidden shadow-2xl shadow-primary/10 border border-white/[0.08] animate-scale-in"
                    ...
                    <div className="px-5 py-4 flex items-center gap-3 border-b border-white/[0.06] bg-gradient-to-r from-primary/10 to-secondary/10">
                        <div className="relative">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-lg shadow-primary/30 text-white">
                                <MessageSquare size={18} />
                            </div>
                            <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-success rounded-full border-2 border-base-100"></span>
                        </div>
                        <div className="flex-1">
                            <h3 className="font-bold text-sm">Trippy 2.0 AI</h3>
                                ...
                                <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${msg.isUser
                                    ...
                                    {msg.navPage && PLATFORM_PAGES[msg.navPage] && (
                                        <button
                                            ...
                                            className="mt-2 btn btn-xs btn-primary btn-gradient gap-2 w-full flex items-center justify-center"
                                        >
                                            <Compass size={12} /> Go to {PLATFORM_PAGES[msg.navPage].name}
                                        </button>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="px-4 pb-2 flex gap-2 overflow-x-auto no-scrollbar">
                            {quickActions.map((action, i) => (
                                <button
                                    ...
                                    className="btn btn-xs btn-outline border-white/10 hover:bg-white/5 whitespace-nowrap text-[11px] rounded-full flex items-center gap-1.5"
                                >
                                    {action.icon}
                                    {action.label}
                                </button>
                    ...
                    <div className="px-4 py-3 border-t border-white/[0.06]">
                        <div className="flex gap-2 items-end">
                            <textarea
                                ...
                                placeholder="Ask Trippy 2.0 anything..."
                            ...
                            <button
                                ...
                                className="btn btn-circle btn-sm bg-gradient-to-r from-primary to-secondary text-white border-0 shadow-lg shadow-primary/20 disabled:opacity-30 disabled:shadow-none hover:scale-105 transition-transform flex items-center justify-center"
                            >
                                <Send size={16} />
                            </button>
                        </div>
